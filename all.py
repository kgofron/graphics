...
def partial_cylinder_intersection(ray_origin, ray_dir, R, theta_min, theta_max, height, cyl_center=(0,0)):
    ...
    # check for z boundary
    z = hit_point[2]

    # check if hit_point is within allowed theta range
...
def main():
    ...
    # TODO calculate theta values from width value and radius, then these values from theta values
    xmin = 5
    ymin = y_c-detect_width/2
    ymax = y_c+detect_width/2
    zmin = -height
    zmax = height

    z = np.linspace(-height, height, 100)
    #theta_grid = np.linspace(0, 2*np.pi, 50)
    theta_grid = np.linspace(np.deg2rad(theta_min_deg), np.deg2rad(theta_max_deg), 50)
    ...
    def run_intensities(event):
        ...
        # Example intensity data — replace with real data
