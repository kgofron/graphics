...
def intensities2detint(cif_path, hkl_path, wavelength, min_intensity, R, cyl_center, ray_origin, zmin, zmax):
    lst = []
    ...
    for idx, refl in df2.iterrows():
        ...
        detthetaz = real2det(o, c, p, t, R, cyl_center, ray_origin)
        if (detthetaz is not None):
            theta = float(detthetaz[0])
            z = float(detthetaz[1])
            if (z<zmax) and (z>zmin):
                lst.append((theta, z, inten, h, k, l))
    if lst is not None:
        return lst
    else:
